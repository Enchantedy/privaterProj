...
#define EVENT_KEY_PRESS_DOWN        (3)         //按下
...
typedef struct
{
    uint32_t timestamp;                         //MCU时间戳 (0.1ms)
    int64_t arm_timestamp;                      //ARM时间戳 (1ms)
    GyroData_t gyro_data;                       //陀螺仪数据
    MainWheelData_t mainwheels_data;            //主轮数据
    AlertMode_t alert_mode;                     //警报模式
    RobotMode_t robot_mode;                     //扫地机模式
    AlongWallData_t alongwall_data;             //沿墙数据
    OmnibearingData_t omnibearing_data;         //全方位数据
    GeologicalDetect_t grdcheck_data;           //地检数据
    SideBrushData_t sidebrush_data;             //边刷数据
    RollBrushData_t rollbrush_data;             //滚刷数据
    PowerSupplyData_t ps_data;                  //供电信息
    InfraredData_t ir_data;                     //红外数据
    VirtualWall_t vw_data;                      //虚拟墙数据
    BumperData_t bumper_data;                   //碰撞信号
    uint16_t fanPulse;                          //风机脉冲
    WakeupTiming_t wakeupTiming;                //唤醒/定时
    PeripheralState_t peripheralState;          //外设检测状态
    PowerState_t powerState;                    //电源检测状态
    AppointmentAction_t appointmentAction;      //预约响应
    uint8_t serialNumber;                       //序列号
    uint8_t iokeyState;                         //GPIO按键状态
    uint8_t reserved1;                          //预留字节1 (临时指定为清除地图标志)
    uint8_t reserved2;                          //预留字节2
    uint8_t reserved3;                          //预留字节3
    uint8_t reserved4;                          //预留字节4
} SensorData_t;

//各模块数据回调函数
typedef void (*SensorDataAllCallback)(SensorData_t *data);
 ...
 * 返 回 值： 无
 **********************************************************************/
extern void UMAPI_ChassisAllDataCallbackRegister(SensorDataAllCallback callback);
