...
//雷达减速&转向
typedef struct radar_WallTurn{
    uint8_t wallObsFront;
    uint8_t wallObsLeft;
    uint8_t wallObsRight;
    uint8_t wallObsFrontSlow;
    uint8_t wallObsLeftSlow;
    uint8_t wallObsRightSlow;
...
struct Sensor
{
    ...
    int leftInfrared_5253;
    int rightInfrared_5253;
    int leftFrontInfrared_5253;
    int rightFrontInfrared_5253;
    int Infrared_5253;
    
    int leftSideBrushElectricity;
    int rightSideBrushElectricity;
    int rollBrushElec;
    radar_SlowTurn_t radarObsState;
    radar_WallTurn_t wallData;
};
...
class Maze
    ...
    public:
        ...
        void RecordXY(Grid cur);
        void setForbidInfo(float esc_x, float esc_y);
        ...
        //vector<vector<std::shared_ptr<Point>>> Map;
        mapRange _map;
        int forceTurn;
        boundary boundMap;
        Grid Seat;
        Grid for_Seat;
        vector<Grid> boundwallOB;

        vector<Grid> filter;
        vector<Grid> wallPoint;
        vector<Grid> linePoint;
        vector<Grid> virPoint;
        //vector<Grid> bumpwallPoint;
        Grid lastUncleanPoint;
        vector<Grid> specialWall;
        Grid wallAim;
        Grid homePoint = {0,0,0,0,0,0,0,0,0};
