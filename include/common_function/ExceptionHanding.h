 ...
 * @LastEditTime : 2022-01-26 21:01:56
...
typedef struct{
    ...
    */
    ...
    int8_t parkTubeCnt;			//上圆管识别
	uint8_t saveUphillCnt;	    //上下坡计数
    int16_t positDiffMinCnt;
    int16_t escUnmovableRateCnt;//角速度不可移动计数
    int16_t escUnmovableCurCnt;//电流不可移动计数
    int16_t escAngleMaxChange;//脱困时最大的变化角度
    int16_t retoNormalCnt;
    int16_t escapPrePitChangCnt;
    int16_t escapPreRolChangCnt;
    int16_t escpAglVeloEffcCnt;

    int16_t escCiffFBase; ///探地基准值
	int16_t escCiffLBase;
	int16_t escCiffRBase;

    float escBasePitRol;//记录机器正常姿态的俯仰横滚
    float escBasePit;//记录机器正常姿态的俯仰
    float escBaseRol;//记录机器正常姿态的横滚
    ...
    long long escInterval;//脱困的时间间隔
    int32_t escActPreAngle;//动作开始时的角度
    int32_t aglDiffCntSpin;
    float fdbRoughlyIncX;   //x大致增量
    float fdbRoughlyIncY;   //y大致增量
    float ldsIncX;    
    float ldsIncY;

    float fdbRoughlyLastIncX;   //x大致增量
    float fdbRoughlyLastIncY;   //y大致增量
    float ldsLastIncX;    
    float ldsLastIncY;
    float dirvPosiChangSum;
    float ldsPosiChangSum;
    float startCheckLdsX;
    float startCheckLdsY; 
    int lidarNormalCnt;
    
    int32_t aglVelEffecCnt;
    int32_t escAglDiffCntSpin;
    uint32_t g_AngleSpeedErrRate;
    uint32_t  g_AngleSpeedErrCnt;
}escapeActionPara_t;
...
typedef struct {
  ...
  enum StuckType
  {
    ...
    ESCAPE_EVENT_TYPE_LIDAR_STUCK,
    ESCAPE_EVENT_TYPE_CLIFF,
  };
  typedef struct SIDEBRUSH_ERROR_MODE
  {
    ...
    {
        DETC_BEHIND_IDLE = 0,
        DETC_BEHIND_ATC, //后退动作检测
        DETC_BEHIND_STOP, //等待一次后退的停止
    ...
    typedef struct {
        bool cTScnSpinF;
        int8_t behindCount;//后退次数计数
        int8_t shortBumpCnt;//短时间碰撞计数
        int8_t stroCliffShortCnt;
        int8_t cTspinLastDir;
        int8_t behindEndF;
        detcBumpState_t detcbehindState;//碰撞脱困检测状态机
        int16_t ciffAllCnt;
        int16_t ciffAllOnCnt;
        int16_t magnVirCnt;
        float trigCiffPreRol;//记录探地触发前3s的横滚数据
        float trigCiffPrePit;//记录探地触发前3s的俯仰数据
        int32_t behindpreDistan;//后退前的距离记录
        long long bumpTimeReco;//碰撞时间记录
        long long behindpreTime;//后退前的时间记录
        
    }bumpEscCheck_t;
    /*碰撞脱困变量*/
    typedef struct {
        int8_t gyroCliffFlag;//用于跌落陀螺仪数据标志位
        int8_t gyroOriginalFlag;
        //Y轴X轴 左边跌落变化判断滤波计数
        int8_t gyroOriginalLeftCnt;
        //Y轴X轴 右边跌落变化判断滤波计数
        int8_t gyroOriginalRightCnt;
    
        //Y轴 上一次数据记录
        int16_t lastGyroOriginalY;
        //X轴 上一次数据记录
        int16_t lastGyroOriginalX;
    ...
    {
        ESCAP_BUMP_IDLE = 0,
        ESCAP_BUMP_FRONT ,//1前进2s，检测
        ESCAP_BUMP_FRONT_WAIT,//2前进2s,等待
        ESCAP_BUMP_SPIN,//3
        ESCAP_BUMP_SPIN_WAIT,//4自旋等待
        ESCAP_BUMP_BEHIND,//5
        ESCAP_BUMP_BEHIND_WAIT,//6
    }escapBumpState_t;
  ...
  class EscapePlan
  {
    ...
    public:
        ...
        //用于监测卡死
        int aimSpdCalAngleData;//通过轮速比计算出来的角速度 单位°/s        
        int fdbSpdCalAngleData;//通过轮速比计算出来的角速度 单位°/s        
        int gyroAngleData;//陀螺仪给出的角速度 单位°/s
       
        //用于监测脱困动作
        int escAimSpdCalAngle = 0;
        int escFdbSpdCalAngle = 0;
        int escGyroAngle = 0;
    ...
    public:
        ...
        */
        ...
        int escapeProcess(int robot_state);
        ...
        */
        ...
        //雷达脱困按时间前进后退
        bool wheelCtrlLidar(int speed, int walkTime);
        ...
        //脱困成功后的自旋                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
        bool spinSuccess(int speed, int dir, float angle);

        //轮速和陀螺仪角度对比检测
        bool wheelToGyroAngleCheck();
        Alert_type alert_detect(); 

        bool GridMacinCheck(void);

        //计算目标角速度
        int calcAimAglVelocity(void);

        //计算反馈角速度
        int calcFdbAglVeloctiy(void);

        //lds位置信息 机器速度反馈  电流 综合判定
        int machinLocationInfoCheck(void);
  };
