...
namespace useerobot
{
    ...
    typedef struct 
    {
        ...
        uint16_t wallForward0Count;

        uint16_t wallForward90Count;

        uint16_t wallForward180Count;

        uint16_t wallForward270Count;
    ...
    typedef struct 
    {
        ...
        uint8_t RestrictedZone;                          //禁区标志位

        uint8_t BoundaryZone;                            //边界标志位
        ...
        WallFallowForbiddenPara_t WallForbidden;

        uint32_t FastWall_Count;
    ...
    private:
        ...
        float rechargeX = 0.0f;
        float rechargeY = 0.0f;
        int forOrVir = 0;
        int virRotateAngle = 0;
        int firstFlag = 0;
        int enterForward = 0;
        float small_place_x = 0.0f, small_place_y = 0.0f;
    public:
        ...
        int8_t rightWallCheck(void);
        void wallCheckInit(void);
        /**
        ...
        bool rotateSlam(float aimForward);
         ...
         * @return: 
         */
        float getBanDis(int dir, int type);
        float getwallBanDis(int dir, int type);
        /*
        地图边界
        */
        float getBoudaryDis(int dir);


        bool isNear(float x, float y, int type);
        bool isVirArea();
        bool isForArea();
        bool isAwayVirArea();
        bool isAwayForArea();
